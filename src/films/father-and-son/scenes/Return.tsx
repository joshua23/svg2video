...
import { Pose, backSkel, blendPose, blendSkel, fk, standHipY } from '../rig/skeleton';
...
import { Geese } from '../scenery/Weather';
...
export const Reunion: React.FC = () => {
  ...
  if (t < 10.4) {
    ...
    const sit = phase(t, 5, 7);
    const up = phase(t, 7.4, 8.8);
    ...
    const step = travel(t, 8.9, 10.4, boatX, 860, 0.5);
    const hop = Math.sin(Math.PI * clamp(invLerp(8.9, 10.2, t))) * 18;
    const w = withStoop(gait(O, gaitPhase(O, step.dist, SHUFFLE), SHUFFLE, step.amount), 0.8);
    him = { b: O, pose: t < 8.9 ? p : blendPose(p, w, step.amount), x: step.x, facing: 1 };
    y = MG - 2 - hop;
    inside = step.x < 800;
  } else {
    inside = false;
    ...
    if (t > ARRIVE - 0.4) {
      ...
      const fs = fk(FATHER, fatherPose(t));
  ...
  const fp = fatherPose(t);
  const dad: ActorState = { b: FATHER, pose: fp, x: FATHER_X, facing: -1 };
  const r = runner(t);
  const follow = lerp(820 + (r.x - 860), r.x + 260, win(t, RUN_T0, 14));
  const camX = t < RUN_T0 ? track(t, [[0, 640], [9, 700], [RUN_T0, 820]]) : lerp(Math.min(follow, FATHER_X - 60), FATHER_X - 70, win(t, 17, ARRIVE));
  const cam: Cam = {
    x: camX,
    y: track(t, [
      [0, 690],
  ...
  return (
    <MarshSet
      ...
      wind={0.28}
      bloom={bloom}
      front={<Card t={t} text="立春" color={look.ink} halo={look.skyLow} t0={1} t1={6.5} />}
...
function fatherPose(t: number): Pose {
  const F = FATHER;
  const d = phase(t, 19.6, 20.8) - phase(t, 23.2, 25);
...
export const Epilogue: React.FC = () => {
  ...
  const cam: Cam = { x: SPOT + 40, y: lerp(560, 575, t / EPILOGUE_SEC), zoom: lerp(1.05, 1.18, t / EPILOGUE_SEC) };
  return (
    <DikeSet
      look={look}
      t={t}
      ...
      waterLevel={0}
      mud={LOOKS.drought.waterNear}
      far={<ReedField t={t} wind={0.3} color={look.hillNear} plume="#f3ead3" grow={1} zNear={1} zFar={20} />}
      crest={(ink) => <Bicycle x={PARKED_BIKE_X} y={GROUND} m={BIKE_M} color={ink} parked tilt={-4} />}
      front={<Geese t={t} x={lerp(2200, -400, clamp((t - 1.5) / 11))} y={230 - t * 3} size={10} color={look.ink} n={9} />}
    />
  );
}
